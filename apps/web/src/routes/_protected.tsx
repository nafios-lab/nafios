...
export const Route = createFileRoute("/_protected")({
  beforeLoad: async ({ location }) => {
    const { user, onboardingCompleted, avatarUrl } = await getOnboardingStatusFn();

    // Session gate for everything protected. No authenticated user → bounce to
    // login, remembering where they were headed. The onboarding-completion gate
    // lives one level down in `_app`, so /onboarding (its sibling) stays
    // reachable while incomplete instead of redirecting to itself.
    if (user === null) {
    ...
    return { user, onboardingCompleted, avatarUrl };
  },
