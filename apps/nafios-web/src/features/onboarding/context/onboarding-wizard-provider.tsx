...
export interface OnboardingWizardProviderProps {
  ...
  /**
   * Hydrated starting data (e.g. a previously-saved Profile step read back on
   * reload). Seeds the wizard so a refresh mid-onboarding does not present an
...
export function OnboardingWizardProvider({
  ...
  const next = useCallback(() => {
    // Two steps → valid indices are 0..1; never advance past the last.
...
/**
 * Small helper hook to expose the `OnboardingWizardContext`.
 */
