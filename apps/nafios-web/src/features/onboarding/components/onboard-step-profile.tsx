...
import { type ProfileValues, profileSchema } from "~/features/onboarding/schemas/onboarding-schema";
...
export function OnboardStepProfile() {
  ...
  // The values just sent, so onSuccess can commit them to wizard state only
  ...
  const form = useForm({
    ...
    onSubmit: ({ value }) => {
      ...
      saveProfile({
        avatar, // data URL, or already-stored path, or undefined — the data layer decides
