import { encodeMonth, formatMonthLong, formatMonthName } from "@nafios/datetime";
 ...
 * the per-status footer tally. No data fetching of its own; its one behaviour is
 * navigation — the whole card links to that month's ledger
 * (`/finance/ledger/$month`), the same target `CreateLedgerForm` pushes to after
 * a create. Colors track the NafiOS theme tokens, not the design mock's literal
 * palette.
 */
export function LedgerDetailCard(summary: LedgerSummaryCard) {
  ...
  return (
    // The whole card is the affordance into the month's ledger — a real Link (not
    // an onClick div) so keyboard, focus and open-in-new-tab come for free. The
    // ring lives on the link, the hover step on the card it wraps.
    <Link
      to="/finance/ledger/$month"
      params={{ month: encodeMonth(summary.month) }}
      aria-label={`Open ledger for ${formatMonthLong(summary.month)}`}
      className="block w-full rounded-xl focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
    >
      <Card
        data-slot="ledger-detail-card"
        className="w-full p-6 transition-[border-color,box-shadow] hover:border-primary/60 hover:shadow-md"
      >
