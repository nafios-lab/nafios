import { type AuthUser, getUser, signUp } from "@nafios/auth-core";
...
/**
 * Encapsulates account creation directly against the Supabase browser client (no
 * server fn). On success the client persists the new session in the browser, so
 * we invalidate the cached `['session']` query before firing `onSuccess`.
 * Duplicate-email is the only user-fixable failure; everything else is a system
 * fault.
 */
export function useAccountSignup(options: UseAccountSignupOptions) {
  ...
  const signupUser = useCallback(
    async (data: AccountSignupFormPayload) => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        // Resume path: a prior attempt may have created the auth user (and
        // session) but failed before the caller navigated on. `getUser`
        // validates the JWT with the Auth server; if it succeeds there is
        // already a signed-in user, so re-running `signUp` would fail with
        // "user already registered". Return the existing user and let the caller
        // proceed. (Ports the server-fn resume logic from apps/web/auth-fns.)
        const existing = await getUser(client);
        if (!existing.error) {
          await invalidateSession(queryClient);
          optionsRef.current.onSuccess?.({ user: existing.data.user });
          return;
        }
        ...
        if (result.error) {
          ...
          const kind =
            result.error.code && DUPLICATE_EMAIL_CODES.has(result.error.code) ? "user" : "system";
        ...
        await invalidateSession(queryClient);
        optionsRef.current.onSuccess?.({ user: result.data.user });
      } catch (cause) {
