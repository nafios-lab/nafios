...
// process-wide in tests/setup.ts. The form signs up via the shared `signUp`
// spy; navigation lands on the shared `navigate` spy.
import { SignupForm } from "../../src/features/auth/components/signup-form.tsx";
import { createWrapper } from "../query-wrapper.tsx";
import { authClient, navigate, resetAuthMocks, signUp } from "../setup.ts";

beforeEach(() => {
  resetAuthMocks();
  signUp.mockResolvedValue({
    error: null,
    data: { user: { id: "u1", email: "new@nafios.local" }, session: {} },
  });
});
afterEach(cleanup);
...
describe("SignupForm — submission", () => {
  test("signs up with the typed credentials and navigates home on success", async () => {
    ...
    expect(signUp).toHaveBeenCalledWith(authClient, {
      email: "new@nafios.local",
