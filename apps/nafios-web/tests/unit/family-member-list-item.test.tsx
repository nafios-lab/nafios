...
describe("FamilyMemberListItem", () => {
  ...
  test("still shows the initials fallback while an avatar image loads", () => {
    // Radix Avatar only swaps in the <img> after it loads; happy-dom never fires
    // that load, so the fallback stays — the branch (member.avatar truthy) is
    // still exercised without asserting an <img> that never mounts.
    render(
      <FamilyMemberListItem
        member={entry({ avatar: "data:image/webp;base64,AAAA" })}
        onEdit={() => {}}
        onDelete={() => {}}
      />,
    );
