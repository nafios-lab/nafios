...
import { db, insertUserProfile, navigate, resetOnboardingMocks } from "../setup.ts";

beforeEach(resetOnboardingMocks);
afterEach(cleanup);

/** Render the step inside the real wizard provider, optionally seeded. The
 *  Family step's useCompleteOnboarding reads useQueryClient() → wrap in the
 *  query provider. */
...
describe("OnboardStepFamily — navigation (full wizard)", () => {
  ...
  test("Skip & finish completes onboarding and redirects to the dashboard", async () => {
    // Default mocks: insertUserProfile resolves → success → navigate to /welcome.
    await skipToFamily();
