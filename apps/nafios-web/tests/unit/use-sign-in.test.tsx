...
// The hook signs in directly against the (mocked) browser client. We drive the
// outcome through the shared `signInWithPassword` spy. The form's submit path is
// covered via LoginForm; this file targets the hook's own surface (user/system
// classification, reset).
import { useSignIn } from "../../src/features/auth/hooks/use-sign-in.ts";
import { createWrapper } from "../query-wrapper.tsx";
import { authClient, resetAuthMocks, signInWithPassword } from "../setup.ts";
...
describe("useSignIn", () => {
  test("signs in against the browser client and fires onSuccess with the user", async () => {
    const onSuccess = mock((_u?: unknown) => {});
    const { result } = renderHook(() => useSignIn({ onSuccess }), { wrapper: createWrapper() });

    await act(async () => {
      await result.current.signIn(credentials);
    });

    expect(signInWithPassword).toHaveBeenCalledWith(authClient, credentials);
  ...
  test("classifies wrong credentials as a user error with an anti-enumeration message", async () => {
    ...
    const { result } = renderHook(() => useSignIn({ onError }), { wrapper: createWrapper() });
