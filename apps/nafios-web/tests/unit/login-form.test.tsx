...
// process-wide in tests/setup.ts. The form signs in via the shared
// `signInWithPassword` spy; navigation lands on the shared `navigate` spy.
import { LoginForm } from "../../src/features/auth/components/login-form.tsx";
import { createWrapper } from "../query-wrapper.tsx";
import { authClient, navigate, resetAuthMocks, signInWithPassword } from "../setup.ts";
...
describe("LoginForm — submission", () => {
  test("signs in with the typed credentials and navigates home on success", async () => {
    ...
    expect(signInWithPassword).toHaveBeenCalledWith(authClient, {
      email: "user@nafios.local",
  ...
  test("does not call the auth client while values are invalid", async () => {
    renderForm();
