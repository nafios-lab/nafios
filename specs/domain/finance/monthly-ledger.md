...
**The month is therefore the ledger's identity.** Because `(user_id, month)` is unique, a user's ledger is fully addressed by its month — the surrogate `id` is a storage detail, not the way the ledger is named. A user asks for "June", not for a UUID. Consequently:

- **Reads and routes are keyed by month.** A ledger is fetched by month; the id is needed only to key the ledger's *relationships* (envelopes, keyed by `ledgerId`), which the caller gets from the ledger it already read.
- **A month with no ledger is a valid, expected answer.** Asking for a month the user never opened is not an error — it is the roll-forward gap ([Section 3](#creation-window--roll-forward)), and the surface answering the question says "no ledger for that month" rather than failing.
...
| `envelopes[]` | Envelope[] | — | All envelopes (line items) for this month. A **relationship, not a stored column** — Envelope is its own entity, keyed by `ledgerId`. See the note below. |
...
> **Note — `envelopes[]` and `derivedMetrics` are conceptual rows, not persisted fields.** The two rows marked *Required: —* describe the ledger as a domain concept; neither is a column on `monthly_ledger`, and neither is a field on the in-memory ledger record. `derivedMetrics` is recomputed on every read (Section 5). `envelopes[]` is a **relationship**: envelopes live in their own table keyed by `ledger_id`, are read separately, and are paired with the ledger by whichever surface needs both (a ledger-detail view, or the metrics engine). A read of the ledger alone is therefore always a complete, honest ledger record — it is simply not a loaded one, and the two are never conflatable.
...
| ------- | ---------- | ----------------- | ------- |
| 0.25    | 2026-08-18 | NafiOS Foundation | **Recorded the month as the ledger's identity.** §1 now states the consequence of the `(user_id, month)` uniqueness constraint that was previously left implicit: a ledger is addressed by its month, not by its surrogate `id`, so reads and routes key on month, the id serves only to reach the ledger's relationships (envelopes), and "no ledger for that month" is a valid answer (the roll-forward gap) rather than an error. No behavior change — this names the addressing model the schema already implied and the finance read surface implements. |
| 0.24    | 2026-08-18 | NafiOS Foundation | **Clarified `envelopes[]` as a relationship, not a persisted field.** §2's field table describes the ledger as a domain concept; the two rows marked *Required: —* (`envelopes[]`, `derivedMetrics`) are not columns on `monthly_ledger` and are not fields on the in-memory ledger record. Envelopes are their own entity keyed by `ledger_id`, read separately and paired with the ledger by whichever surface needs both. No behavior change — this documents the normalization the schema already implemented, and unblocks removing the vestigial `envelopes` field from the implementation type. |
