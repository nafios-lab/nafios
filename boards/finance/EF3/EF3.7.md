...
export type CreateLedgerRejectionReason =
  ...
  | "overspend_warning" // EF3.5 amber zone, confirmed === false
...
2. **MaxCapped guardrail (pure — EF3.5).** Call `validateMaxCapped({ openingBalance, maxCapped, confirmed })`. On `{ ok: false, reason }` return `{ ok: false, reason, guardrail }` verbatim — the reason is exactly EF3.5's `overspend_warning` (amber, not confirmed) or `exceeds_hard_cap` (blocked, no override), and the `guardrail` (with `savingsDraw` / `hardCap`) travels so EF3.12 can render the sheet/message. The command does **not** re-derive the zones.
...
1. **Deterministic input/context failure → `{ ok: false }` result.** `negative_amount`, `overspend_warning`, `exceeds_hard_cap`, `month_not_openable`. These are _user_ input the UI renders — no exception, no write. Same channel as EF3.5's `validateMaxCapped` result.
...
| 6   | Open current month, amber `maxCapped 7500.00 > opening 7152.35`, `confirmed:false` | `{ ok:false, reason:'overspend_warning', guardrail.zone:'amber', savingsDraw '347.65' }`; no write |
...
- [ ] **AC2** — All three pre-write checks are enforced **in the command, before any write**, and each failure returns `{ ok: false }` with **no DB mutation**: `negative_amount` (opening/maxCapped < 0), the EF3.5 guardrail (`overspend_warning` / `exceeds_hard_cap`, carrying `guardrail`), and `month_not_openable` (EF3.4 openable set) (rows 5–12).
