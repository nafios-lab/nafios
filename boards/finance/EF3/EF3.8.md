...
| ------- | ---------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0.2     | 2026-07-09 | Hanafi Yakub      | **Implemented.** Added `src/internal/mappers/envelope.mapper.ts` (owns the `statusToDb`/`statusFromDb` `carried_over ↔ carried-over` seam — the only `carried_over` in the package), `src/internal/repositories/envelope.repo.ts` (`createEnvelopeRepository`), and `src/internal/commands/envelope-commands.ts` (`createEnvelopeCommands`); extended `errors.ts` with `23503 → foreign_key_violation`; barrel-exported the command write surface. Mocked-SDK unit tests (`tests/unit/`, in the coverage gate, 100%) + repo-root live matrices `tests/integration/envelope.repo.test.ts` (§6.1) and `tests/integration/envelope-commands.test.ts` (§6.2). **Reconciliations with the live schema/repo conventions** (the §2 signatures are unchanged — only names differ): files follow the established `*.repo.ts` / `*.mapper.ts` convention (not `envelope-repository.ts`); integration matrices live at repo-root `tests/integration/` per the EF3.6/3.7 test-lane decision (ADR-0020), not in-package. The envelope's person ref is column **`linked_member_id` → `family_members`** (EF1.4 reused `family_members`; no `person` table), mapped to the domain's `linkedPerson`. Real FK names are **`fk_envelope_category`** / **`fk_envelope_ledger`** (the EF1 ownership-hardening migration replaced the generated `envelope_*_id_fkey` with composite `(col, user_id)` FKs), so §6.1 rows 10–11 assert those. `bun run check` green (typecheck + gated unit tests + lint/format). |
