...
> - **Blocks:** [EF3.12](EF3.12.md) — the onboarding wiring that invokes this API (implements the §4.5 integration contract)
...
> **Consumed by:**
>
> - **[EF3.12](EF3.12.md) — the onboarding wiring** (primary consumer) — `import { createServiceClient, provisionDefaultCategories } from "@nafios/finance"` and calls `provisionDefaultCategories(createServiceClient(), newUserId)` as one step in the new-user onboarding flow (story-map S1: *"my workspace comes pre-loaded with a sensible default category set"*). This ticket ships the finance-side API + the integration contract (§4.5); **[EF3.12](EF3.12.md) owns the wiring** that calls it from `apps/web`'s onboarding flow (an `apps/web` monorepo edit — there is **no** separate auth/platform repo). **EF3.9 blocks EF3.12.**
