...
import { CalendarLogo } from "../../src/components/logo/calendar-logo.tsx";
...
describe("CalendarLogo", () => {
  test("renders an aria-hidden svg with the default sizing class", () => {
    const { container } = render(<CalendarLogo />);
  ...
  test("merges a custom className", () => {
    const { container } = render(<CalendarLogo className="text-red-500" />);
    expect(svg(container).getAttribute("class")).toContain("text-red-500");
  });

  test("forwards arbitrary svg attributes", () => {
    const { container } = render(<CalendarLogo data-testid="calendar" role="img" />);
    const el = svg(container);
    expect(el.getAttribute("data-testid")).toBe("calendar");
