...
import { FinanceLogo } from "../../src/components/logo/finance-logo.tsx";
...
describe("FinanceLogo", () => {
  ...
  test("merges a custom className", () => {
    const { container } = render(<FinanceLogo className="text-red-500" />);
    expect(svg(container).getAttribute("class")).toContain("text-red-500");
  });

  test("forwards arbitrary svg attributes", () => {
    const { container } = render(<FinanceLogo data-testid="finance" role="img" />);
    const el = svg(container);
    expect(el.getAttribute("data-testid")).toBe("finance");
