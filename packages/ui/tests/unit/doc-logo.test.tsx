...
import { DocLogo } from "../../src/components/logo/doc-logo.tsx";
...
describe("DocLogo", () => {
  test("renders an aria-hidden svg with the default sizing class", () => {
    const { container } = render(<DocLogo />);
  ...
  test("merges a custom className", () => {
    const { container } = render(<DocLogo className="text-red-500" />);
    expect(svg(container).getAttribute("class")).toContain("text-red-500");
  });

  test("forwards arbitrary svg attributes", () => {
    const { container } = render(<DocLogo data-testid="doc" role="img" />);
    const el = svg(container);
    expect(el.getAttribute("data-testid")).toBe("doc");
