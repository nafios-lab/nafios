...
import { BudgetingLogo } from "../../src/components/logo/budgeting-logo.tsx";
...
describe("BudgetingLogo", () => {
  ...
  test("merges a custom className", () => {
    const { container } = render(<BudgetingLogo className="text-red-500" />);
    expect(svg(container).getAttribute("class")).toContain("text-red-500");
  });

  test("forwards arbitrary svg attributes", () => {
    const { container } = render(<BudgetingLogo data-testid="budgeting" role="img" />);
    const el = svg(container);
    expect(el.getAttribute("data-testid")).toBe("budgeting");
